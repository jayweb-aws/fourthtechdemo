...
import { useDeleteResourceMutation } from "../../../../../feature/api/fourthit/resource/resourceApi";
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "icon",
        Cell: (row: any) => {
          const { icon } = row?.cell?.row?.original;
          return (
            <div className="flex items-center space-x-2">
              <div className="h-[40px] w-[40px] lg:h-[60px] lg:w-[60px] ">
                <img className="h-full w-full rounded-md" src={icon} alt="" />
              </div>
            </div>
          );
        },
      ...
      {isOpen && modal === "delete" && (
        ...
        >
          <DeleteModal
            variant="Category"
            closeModal={closeModal}
            rtkMutation={useDeleteResourceMutation}
            title={"Do you want delete this resource !"}
            id={id}
            successMsg="Resource delete success"
          />
        </ModalDialog>
      )}
      {isOpen && modal === "edit" && (
