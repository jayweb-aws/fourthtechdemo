...
const CourseCard = ({ item }: { item: any }) => {
  return (
    <Link href={"/dashboard/course/[id]"} as={`/dashboard/course/${item.id}`}>
      <div className="w-[275px]  cursor-pointer overflow-hidden shadow rounded-lg">
        <div className=" bg-[#FFFFFF] h-full rounded-lg ">
          <div className="!w-full bg-[#FFFFFF]">
            <Image
              src={item ? item.courseImage : img}
          ...
          <div className="bg-[#FFFFFF] p-3 ">
            <h4 className="text-lg font-medium  overflow-auto">
              {item == null ? "Empty" : item?.title}
            </h4>
            <div className="">
              <div className="flex justify-between pb-2">
                <span className="text-[#8A92A6]">
                  <Image src={star} />
                  {item == null ? 0 : item?.ratingsAverage}
                </span>
              </div>
              ...
              <span className="text-[#8A92A6]">
                {item?.completed === null && 0}100% Completed
              </span>
            </div>
          </div>
        </div>
